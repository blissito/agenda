...
describe("Email Templates", () => {
  describe("reminderTemplate", () => {
    it("should generate valid HTML with all parameters", async () => {
      const { default: reminderTemplate } = await import(
        "~/utils/emails/reminderTemplate"
      )
  ...
  describe("appointmentCustomerTemplate", () => {
    it("should generate confirmation email with all details", async () => {
      const { default: appointmentTemplate } = await import(
        "~/utils/emails/appointmentCustomerTemplate"
      )
...
describe("WhatsApp Service", () => {
  it("should throw WhatsAppNotConfiguredError when not configured", async () => {
    const { sendWhatsAppMessage, WhatsAppNotConfiguredError } = await import(
      "~/.server/whatsapp"
    )
...
describe("Messenger Service", () => {
  it("should throw MessengerNotConfiguredError when not configured", async () => {
    const { sendMessengerMessage, MessengerNotConfiguredError } = await import(
      "~/.server/messenger"
    )
...
describe("Event Action Tokens", () => {
  ...
  it("should generate and verify event action token", async () => {
    const { generateEventActionToken, verifyEventActionToken } = await import(
      "~/utils/tokens"
    )
  ...
  it("should return null for tampered token", async () => {
    ...
    const tamperedToken =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJldmVudElkIjoiZmFrZSIsImN1c3RvbWVySWQiOiJmYWtlIiwiYWN0aW9uIjoiY29uZmlybSIsImlhdCI6MTYwMDAwMDAwMH0.tampered"
...
describe("negativeReviewTemplate", () => {
  it("should generate valid HTML with all parameters", async () => {
    const { default: negativeReviewTemplate } = await import(
      "~/utils/emails/negativeReviewTemplate"
    )
...
describe("Review Stats Calculation", () => {
  ...
  const calculateStats = (
    reviews: { rating: number; customerId: string }[],
  ) => {
