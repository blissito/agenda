const STATS_BELT_STYLES = `
  @keyframes stats-belt-bar-fill { from { width: 0 !important } }
  .stats-belt-bar span { animation: stats-belt-bar-fill 1.4s ease-out }
...
export function StatsBelt() {
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: STATS_BELT_STYLES }} />
          ...
          <div className="relative grid grid-cols-1 md:grid-cols-[1.05fr_1.35fr] items-center gap-8 md:gap-14">
            <div className="relative z-[2]">
              <div className="inline-flex items-center gap-2 pl-2.5 pr-3.5 py-1.5 rounded-full bg-white/5 border border-white/10 text-[11px] font-semibold tracking-widest uppercase text-white/80">
                <span className="w-1.5 h-1.5 rounded-full bg-brand_yellow shadow-[0_0_10px_#FFD75E]" /> filosofía
              </div>
              ...
              <p className="text-[15px] leading-relaxed text-white/70 mb-6 max-w-[460px]">
                Automatizamos lo aburrido — agendar, responder lo repetido, pasar reportes — para que tú hagas lo único que tu cliente no puede conseguir en otro lado: <b className="text-white font-bold">tú</b>.
              </p>
            ...
            <div className="relative z-[2] grid grid-cols-1 md:grid-cols-3 gap-3.5">
              <BeltStat
                ...
                value={<><span className="text-[26px] opacity-70 -mr-0.5">+</span>9<span className="text-[20px] font-bold opacity-75 ml-1">h</span></>}
              ...
              <BeltStat
                ...
                value={<>87<span className="text-[20px] font-bold opacity-75 ml-1">%</span></>}
              ...
              <BeltStat
                ...
                value={<>2<span className="text-[20px] font-bold opacity-75 ml-1">min</span></>}
...
function BeltAv({ bg, src, children }: { bg?: string; src?: string; children?: React.ReactNode }) {
  return (
    <div className={`${bg ?? ""} w-[34px] h-[34px] rounded-full border-2 border-brand_dark -ml-2 first:ml-0 grid place-items-center text-xs font-bold text-white overflow-hidden`}>
      {src ? <img src={src} alt="" className="w-full h-full object-cover" /> : children}
    </div>
  )
}
...
function BeltStat({
  ...
  return (
    <div className="relative rounded-[22px] px-4 py-5 backdrop-blur-[10px] transition-all duration-300 hover:-translate-y-1 overflow-hidden bg-gradient-to-b from-white/5 to-white/5 border border-white/10 hover:border-white/20">
      <div className={`w-9 h-9 rounded-[10px] grid place-items-center mb-3.5 bg-white/10 ${palette.text} [&>svg]:w-[18px] [&>svg]:h-[18px]`}>
        {icon}
      </div>
      <div className="font-satoBold font-extrabold text-[52px] tracking-tight leading-[.95] flex items-baseline gap-0.5 text-white">
        {value}
      </div>
      <div className="text-white/75 text-[13px] leading-snug mt-2.5 font-medium max-w-[170px]">{label}</div>
      <div className="h-1 bg-white/10 rounded-full mt-4 overflow-hidden stats-belt-bar">
        <span className={`block h-full rounded-full ${palette.bar}`} style={{ width: barWidth }} />
      </div>
