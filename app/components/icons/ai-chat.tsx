...
export const AiChatIllustration = ({ className }: { className?: string }) => (
  <div className={twMerge("flex flex-col gap-3 max-w-xs ml-auto", className)}>
    {messages.map((msg, i) => (
      <div
        key={i}
        className={twMerge(
      ...
      >
        {msg.from === "ai" && (
          <img
            ...
            alt="Nik"
            className="w-6 h-6 rounded-full shrink-0 mr-2 mt-1 -scale-x-100"
          />
        )}
        <div
          className={twMerge(
