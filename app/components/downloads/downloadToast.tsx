      ...
      <div className="overflow-hidden rounded-2xl bg-white border border-slate-200 shadow-[0_18px_55px_rgba(0,0,0,0.25)]">
        ...
        <div className="px-5 py-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[14px] text-brand_gray">{statusText(status)}</p>
          </div>
...
function buildClientsCsv(clients: ClientForCsv[]): string {
  ...
  const rows = clients.map((c) => {
    ...
    const puntos =
      typeof c.points === "number" && !Number.isNaN(c.points) ? c.points : ""
    const citas =
      typeof c.eventCount === "number" && !Number.isNaN(c.eventCount)
        ? c.eventCount
        : ""
...
export function useDownloadToast({
  ...
  const startDownload = React.useCallback(() => {
    ...
    const safeOrg =
      (orgName || "empresa").trim().replace(/\s+/g, "-").toLowerCase() ||
      "empresa"
  ...
  const companyName = orgName?.trim() || "Empresa"
...
function sanitizeFileName(value: string) {
  return (
    value
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      // biome-ignore lint/suspicious/noControlCharactersInRegex: sanitize filenames
      .replace(/[<>:"/\\|?*\u0000-\u001F]/g, "-")
      .replace(/\s+/g, "_")
      .replace(/-+/g, "-")
      .replace(/_+/g, "_")
