...
export async function deleteLevel(levelId: string) {
  ...
  if (customers > 0) {
    return {
      error: `No se puede eliminar: ${customers} cliente(s) tienen este nivel. Reasígnalos primero.`,
    }
  }
 ...
 */
export async function getLevelDiscount(params: {
  customerId: string
  serviceId: string
}) {
  ...
  if (
    ...
    !level.serviceIds.includes(params.serviceId)
  ) {
    return null
  }
...
export async function deleteReward(rewardId: string) {
  ...
  if (redemptions > 0) {
    return {
      error:
        "No se puede eliminar una recompensa con canjes. Desactivala en su lugar.",
    }
  }
