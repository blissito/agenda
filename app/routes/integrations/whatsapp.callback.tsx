...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  // Handle OAuth errors
  if (error) {
    console.error("[WhatsApp OAuth] Error:", error, url.searchParams.get("error_description"));
    throw redirect(`/dash/ajustes?error=whatsapp_oauth_failed&message=${encodeURIComponent(error)}`);
  }
  ...
  try {
    ...
    if (tokenData.error) {
      ...
      throw redirect(`/dash/ajustes?error=whatsapp_token_failed&message=${encodeURIComponent(tokenData.error.message)}`);
    }
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="text-center">
        <h1 className="text-xl font-bold mb-4">Procesando conexión con WhatsApp...</h1>
