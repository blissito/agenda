...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  if (error) {
    ...
    throw redirect(
      `/dash/ajustes?error=messenger_oauth_failed&message=${encodeURIComponent(error)}`,
    )
  }
  ...
  try {
    ...
    if (tokenData.error) {
      ...
      throw redirect(
        `/dash/ajustes?error=messenger_token_failed&message=${encodeURIComponent(tokenData.error.message)}`,
      )
    }
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="text-center">
        <h1 className="text-xl font-bold mb-4">
          Procesando conexión con Messenger...
        </h1>
