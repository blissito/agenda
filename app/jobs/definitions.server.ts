...
agenda.define("send-reminder", async (job) => {
  ...
  if (!event || event.status === "CANCELLED" || event.reminderSentAt) {
    console.log(`[send-reminder] Skipping event ${eventId}: not found, cancelled, or already sent`);
    return;
  }

  if (!event.customer || !event.service) {
    console.log(`[send-reminder] Skipping event ${eventId}: missing customer or service`);
    return;
  }
  ...
  try {
    ...
    console.error(`[send-reminder] Failed to send reminder for event ${eventId}:`, error);
...
agenda.define("send-survey", async (job) => {
  ...
  if (!event || event.status === "CANCELLED" || event.surveySentAt) {
    console.log(`[send-survey] Skipping event ${eventId}: not found, cancelled, or already sent`);
    return;
  }

  if (!event.customer || !event.service) {
    console.log(`[send-survey] Skipping event ${eventId}: missing customer or service`);
    return;
  }
  ...
  try {
    ...
    console.error(`[send-survey] Failed to send survey for event ${eventId}:`, error);
    throw error;
  }
});
...
export const scheduleReminder = async (
  ...
  const reminderTime = new Date(eventStart.getTime() - reminderHours * 60 * 60 * 1000);

  // Don't schedule if reminder time is in the past
  if (reminderTime <= new Date()) {
    console.log(`[scheduleReminder] Skipping event ${eventId}: reminder time already passed`);
    return;
  }

  await agenda.schedule(reminderTime, "send-reminder", { eventId });
  console.log(`[scheduleReminder] Scheduled reminder for event ${eventId} at ${reminderTime.toISOString()}`);
};
...
export const scheduleSurvey = async (
  ...
  const surveyTime = new Date(eventEnd.getTime() + SURVEY_DELAY_MINUTES * 60 * 1000);

  // Don't schedule if survey time is in the past
  if (surveyTime <= new Date()) {
    console.log(`[scheduleSurvey] Skipping event ${eventId}: survey time already passed`);
    return;
  }

  await agenda.schedule(surveyTime, "send-survey", { eventId });
  console.log(`[scheduleSurvey] Scheduled survey for event ${eventId} at ${surveyTime.toISOString()}`);
};
